...
# This is an updated Moses tokenizer taht takes into account indic languages ()
# "hi","ur","ml","te","ta","bn"
# updated by BP 6/12/2016
#
...
# Changed the directory to an absolute path
my $mydir = "/home/smt/mosesdecoder/scripts/share/nonbreaking_prefixes";
...
sub tokenize
{
    ...
    if ($language eq "hi" || $language eq "ur" || $language eq "ml" || $language eq "te" || $language eq "ta" || $language eq "bn" || $language eq "en") {
...
sub tokenize_indic
{
    # Set of rules that do not break indic strings (unfortnuatelly the standard
    # tokenizer breaks words around vowels
    # Adapted from tokenize_penn
    ...
    $text =~ s=\.\.\.= _ELLIPSIS_ =g; # save ellipsis
     $text =~ s=…= _ELLIPSIS_ =g; # save devanagari ellipsis
    ...
    # Separate hyphen and dot (when attached to both sides)
#print STDERR "BP:$text:\n";
    $text =~ s=([^[:punct:][:space:]])([\.-])([^[:punct:][:space:]])=$1 $2 $3=g;

    # Normalize numerals, see https://en.wikipedia.org/wiki/Indian_numerals
    $text =~ tr/٠-٩/0-9/; # ar
    $text =~ tr/०-९/0-9/; # hi
    $text =~ tr/০-৯/0-9/; # bn
    $text =~ tr/૦-૯/0-9/;
    $text =~ tr/੦-੯/0-9/; # punjabi
    $text =~ tr/୦-୯/0-9/; # Odia
    $text =~ tr/౦-౯/0-9/; # Telugu
    $text =~ tr/೦-೯/0-9/; # ka
    $text =~ tr/௦-௯/0-9/; # ta
    $text =~ tr/൦-൯/0-9/; # ma
...
#    print STDERR "BP:$text;",(scalar %NONBREAKING_PREFIX),"\n";
    #word token method
    if ((scalar keys %NONBREAKING_PREFIX) < 1) 
    {
	# First off, add a space to the beginning and end of each line, to reduce
	# necessary number of regexps.
	$text =~ s=$= =;
    ...
    $text =~ s=[꣎।॥।  …]*$==g; # Specific end markers for Indic
    $text =~ s=([\x{0609}\x{060A}\x{060C}\x{060D}\x{061B}\x{066A}\x{066D}])= $1 =g; # arabic punctuation
    $text =~ s=([\x{06d4}\x{061f}\x{061E}\x{06D4}])==g; # ARABIC (QUESTION MARK|triple dot|full stop)
